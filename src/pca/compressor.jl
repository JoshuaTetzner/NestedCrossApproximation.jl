...
function getcompressedmatrix_rm(
    ...
    maxrank = min(
        Int(
            round(
                length(test_idcs) * length(trial_idcs) /
                (length(test_idcs) + length(trial_idcs)),
            ),
        ),
        compressor.maxrank,
    ...
    lm = FastBEAST.LazyMatrix(matrixassembler, test_idcs, trial_idcs, K)

    pivstrat = compressor.columnpivstrat
    if compressor.columnpivstrat isa PCAPivoting
        pivstrat = PCAPivoting(
            compressor.columnpivstrat.fct,
            refcenter,
            compressor.columnpivstrat.pos[trial_idcs],
        )
    elseif compressor.columnpivstrat isa PCAPivoting2
        pivstrat = PCAPivoting2(refcenter, compressor.columnpivstrat.pos[trial_idcs])
    end

    @time U, V, rows, cols = pca_rm(lm, am, pivstrat; maxrank=maxrank, tol=compressor.tol)
    @views MU = U * V
    @views MV = U[rows, :] * V

    return MatrixBlock{I,K,ClusterMatrix{I,K}}(
        ClusterMatrix(MU, MV, rows, cols), test_idcs, trial_idcs
    )
end
...
function getcompressedmatrix_cm(
    ...
    elseif compressor.rowpivstrat isa PCAPivoting2
        pivstrat = PCAPivoting2(refcenter, compressor.rowpivstrat.pos[test_idcs])
    end

    U, V, rows, cols = pca_cm(lm, am, pivstrat; maxrank=maxrank, tol=compressor.tol)
